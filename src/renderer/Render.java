...
public class Render {
     ...
     */
    private void castRay(int nX, int nY, int col, int row, double alpha) {
        Ray ray = camera.constructRayThroughPixel(nX, nY, col, row);
        Color color = tracer.traceRay(ray,alpha);
     ...
     */
    private void renderImageThreaded(double alpha) {
        ...
        for (int i = threadsCount - 1; i >= 0; --i) {
            threads[i] = new Thread(() -> {
                ...
                while (thePixel.nextPixel(pixel))
                    castRay(nX, nY, pixel.col, pixel.row,  alpha);
            });
        }
        // Start threads
    ...
    public void renderImage(double alpha) {
        ...
        if (threadsCount == 0)
            for (int i = 0; i < nY; ++i)
                for (int j = 0; j < nX; ++j)
                    castRay(nX, nY, j, i, alpha);
        else
            renderImageThreaded( alpha);
    }
